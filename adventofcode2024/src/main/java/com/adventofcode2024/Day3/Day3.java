...
public class Day3 extends Aoc {
    ...
    protected String part1(ArrayList<String> input) {
        return String.valueOf(Pattern.compile("mul\\(\\d+,\\d+\\)")
                .matcher(input.stream().map(Object::toString).collect(Collectors.joining("")))
                .results()
                .map(s -> s.group()).collect(Collectors.toList())
                .stream()
                .mapToInt(s -> Integer.parseInt(s.substring(4, s.length() - 1).split(",")[0])
                        * Integer.parseInt(s.substring(4, s.length() - 1).split(",")[1]))
                .sum());
    }
